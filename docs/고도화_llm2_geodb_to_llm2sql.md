...
| 고도화 시스템 | `llm2sql` (v0.1.4) — 자연어→SQL→한국어 답변 |
...
| 0.1.4 | 지명 사전(법정/행정), 공간·임계·행정동 목록, 의도분류/답변 템플릿 최적화, 지도 레이어 발행 |
| 1.4.0 | 지도 3분할 고도화, Head/Bottom 프레임, 데이터 관리 골격, Identify 설명, 분석 레이어 재사용 |
