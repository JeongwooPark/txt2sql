...
**현재 버전: 0.3.2** (SQP Plan v1.1 + Query Contract·plan repair, `SEMANTIC_PLAN_MODE` 기본 `hybrid`)  
근거: `pyproject.toml`의 `version = "0.3.2"`, FastAPI 앱(`txt2sql/webapp/app.py`)의 `version="0.3.2"`. SQP 기본값은 `txt2sql/config.py`와 `.env.example`의 `hybrid`. 0.3.2 변경은 `docs/20260904_txt2sql_v0.3.2.md`.
...
**txt2sql 0.3.2**는 부산 GIS(건물·행정구역·기초구역·산업단지)를 **한국어 자연어**로 물어보면, 내부에서 **PostgreSQL + PostGIS용 SELECT**를 만들고 실행한 뒤 **한국어 답변**을 돌려주는 도구다. 로컬 **Ollama**가 SQL·서술·임베딩을 맡고, 선택적으로 **GeoServer**에 임시 레이어를 올려 지도에 그린다. 라우터 미적중 질의는 기본으로 Semantic Query Plan(`hybrid`)을 실행하며, Plan→SQL은 Query Contract 검증과 contract 기반 repair를 거친다.
