...
**현재 버전: 0.2.3** (SQP Plan v1.1, `SEMANTIC_PLAN_MODE` 기본 `hybrid`)  
근거: `pyproject.toml`의 `version = "0.2.3"`, FastAPI 앱(`llm2sql/webapp/app.py`)의 `version="0.2.3"`. SQP 기본값은 `llm2sql/config.py`와 `.env.example`의 `hybrid`.
...
**llm2sql 0.2.3**은 부산 GIS(건물·행정구역·기초구역·산업단지)를 **한국어 자연어**로 물어보면, 내부에서 **PostgreSQL + PostGIS용 SELECT**를 만들고 실행한 뒤 **한국어 답변**을 돌려주는 도구다. 로컬 **Ollama**가 SQL·서술·임베딩을 맡고, 선택적으로 **GeoServer**에 임시 레이어를 올려 지도에 그린다. 라우터 미적중 질의는 기본으로 Semantic Query Plan(`hybrid`)을 실행한다.
...
이 문서는 **0.2.3 코드가 지금 어떻게 도는지**를 시나리오로 설명하는 쪽에 맞춰 두었다. SQP v1.1에서 `SEMANTIC_PLAN_MODE` 기본값은 `hybrid`이다. 실패·clarify·품질 미달 시 RAG로 내려간다. 적용·롤백은 `docs/implementation/sqp_v11_migration.md`, `docs/implementation/sqp_v11_rollback.md`를 본다.
