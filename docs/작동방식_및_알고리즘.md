...
**현재 버전: 0.2.1**  
근거: `pyproject.toml`의 `version = "0.2.1"`, FastAPI 앱(`llm2sql/webapp/app.py`)의 `version="0.2.1"`.
...
**llm2sql 0.2.1**은 부산 GIS(건물·행정구역·기초구역·산업단지)를 **한국어 자연어**로 물어보면, 내부에서 **PostgreSQL + PostGIS용 SELECT**를 만들고 실행한 뒤 **한국어 답변**을 돌려주는 도구다. 로컬 **Ollama**가 SQL·서술·임베딩을 맡고, 선택적으로 **GeoServer**에 임시 레이어를 올려 지도에 그린다.

한 문장으로 줄이면 다음과 같다.

> 질문 → (안내 / 메타 / 모호 확인 / 특징·비교 / 순위비교 / 규칙 SQL / RAG+LLM) → DB 조회 → 한국어 답변 → (가능하면) 차트 제안 · 지도 레이어
...
이 문서는 **0.2.1 코드가 지금 어떻게 도는지**를 시나리오로 설명하는 쪽에 맞춰 두었다.
