...
run_ask
  ...
  9. _ask_inner
       ...
       9i. 미적중이면                 SQP (shadow/hybrid) → 실패 시 run_rag_sql
...
| 모드 | 동작 |
|------|------|
| `off` (기본) | 바로 `run_rag_sql`. 0.2.1과 같다 |
| `shadow` | Plan 생성·검증·컴파일만. 답은 여전히 RAG |
| `hybrid` | Plan SQL을 실행. 실패·품질 미달·미지원이면 RAG |
...
| `SEMANTIC_PLAN_MODE` | `shadow` | SQP `off` / `shadow` / `hybrid` |
...
이 문서는 **0.2.2 코드가 지금 어떻게 도는지**를 시나리오로 설명하는 쪽에 맞춰 두었다. `SEMANTIC_PLAN_MODE` 기본값은 `shadow`이며, `hybrid`는 승격 gate를 통과하기 전에는 기본값이 아니다. 실패 시 RAG로 내려간다. 적용·롤백은 `docs/implementation/sqp_v11_migration.md`, `docs/implementation/sqp_v11_rollback.md`를 본다.
