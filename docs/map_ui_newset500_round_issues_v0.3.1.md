...
- 이 문서는 **0.3.1 안정화 브랜치**용 이슈 기록이다 (역사 문서).
- **0.3.2**에서 Semantic Architecture v2·Query Contract·plan repair가 `master`에 통합되었다 (`docs/20260904_txt2sql_v0.3.2.md`).
- Round3 에이전트는 평가·결과 캔버스까지 완료(244/500, 48.8%). 이후 newset500·골드테스트2 기준으로 재측정한다.
