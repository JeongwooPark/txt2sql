...
- `Q338` 금정구 서동 주거용 중 지하층 있는 채수  
  gold=`358채` reason=`count-mismatch gold=358 pred=[6341.0, 6341.0]` route=`semantic_plan_count`

### ENTITY_SELECTION_ERROR

- `Q311` 동래구 허가일과 사용승인일 연도 차이가 3년 이상인 집합건축물 채수  
  gold=`360채` reason=`engine-fail:unsupported_coverage: building age / 사용승인` route=`None`
...
- `Q323` 동래구 사직동 상업용 중 허가 1990년대인 채수  
  gold=`163채` reason=`engine-fail:numeric operator on text field: permit_date` route=`None`
...
- `Q330` 금정구 허가일과 사용승인일의 연도 차이가 5년 이상인 건물 수  
  gold=`278채` reason=`engine-fail:unsupported_coverage: building age / 사용승인` route=`None`
